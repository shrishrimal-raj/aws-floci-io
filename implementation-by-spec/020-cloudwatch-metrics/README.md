...
> Custom metrics, alarms, dashboards, EMF helpers, dimensions, and cleanup.
...
docker compose up -d
pnpm install
cd implementation-by-spec/020-cloudwatch-metrics
pnpm setup
pnpm seed
pnpm test
pnpm cleanup
...
- `src/client.ts` - CloudWatch Metrics SDK v3 client for Floci (`http://localhost:4566`).
- `src/use-cases/metrics.ts - metric data, alarms, dashboards, delete alarms, EMF helper.`
...
await putMetric("App", "OrdersCreated", 1, "Count", { Service: "orders" });
```
...
- Control dimension cardinality, alarm actions, dashboards, SLOs, cost awareness.
