# 026 - ElastiCache Redis

> Managed Redis with cluster lifecycle helpers, Redis URLs, and cache key conventions.
...
docker compose up -d
pnpm install
cd implementation-by-spec/026-elasticache-redis
pnpm setup
pnpm seed
pnpm test
pnpm cleanup
...
- `src/client.ts` - ElastiCache Redis SDK v3 client for Floci (`http://localhost:4566`).
- `src/use-cases/redis.ts - create/describe/delete Redis clusters plus URL/cache key helpers.`
...
const key = cacheKey("orders", "dev", "cart:1");
```
...
- Configure subnet groups, auth token, TLS, eviction policy, replication, alarms.
