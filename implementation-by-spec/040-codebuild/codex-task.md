# Codex CLI task - CodeBuild

You are completing the `implementation-by-spec/040-codebuild/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to CodeBuild.
...
docker compose up -d
pnpm install
cd implementation-by-spec/040-codebuild
pnpm setup
pnpm test
```
