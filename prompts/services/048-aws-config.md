# Codex CLI task - AWS Config

You are completing the `implementation-by-spec/048-aws-config/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to AWS Config.
...
docker compose up -d
pnpm install
cd implementation-by-spec/048-aws-config
pnpm setup
pnpm test
```
