# Codex CLI task - CodeDeploy

You are completing the `implementation-by-spec/041-codedeploy/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to CodeDeploy.
...
docker compose up -d
pnpm install
cd implementation-by-spec/041-codedeploy
pnpm setup
pnpm test
```
