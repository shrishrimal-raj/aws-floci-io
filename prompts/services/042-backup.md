# Codex CLI task - AWS Backup

You are completing the `implementation-by-spec/042-backup/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to AWS Backup.
...
docker compose up -d
pnpm install
cd implementation-by-spec/042-backup
pnpm setup
pnpm test
```
