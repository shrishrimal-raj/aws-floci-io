# Codex CLI task - Resource Groups Tagging API

You are completing the `implementation-by-spec/051-resource-groups-tagging-api/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to Resource Groups Tagging API.
...
docker compose up -d
pnpm install
cd implementation-by-spec/051-resource-groups-tagging-api
pnpm setup
pnpm test
```
