...
> Modern SES API with configuration sets, contact lists, simple sends, and cleanup.
...
docker compose up -d
pnpm install
cd implementation-by-spec/022-ses-v2
pnpm setup
pnpm seed
pnpm test
pnpm cleanup
...
- `src/client.ts` - SES v2 SDK v3 client for Floci (`http://localhost:4566`).
- `src/use-cases/email-v2.ts - configuration sets, contact lists, send email, cleanup.`
...
await sendEmailV2("from@example.com", ["to@example.com"], "Hello", "Body", "app-config");
```
...
- Configure event destinations, suppression lists, contact topics, reputation monitoring.
