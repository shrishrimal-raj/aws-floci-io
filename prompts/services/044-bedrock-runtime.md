# Codex CLI task - Bedrock Runtime

You are completing the `implementation-by-spec/044-bedrock-runtime/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to Bedrock Runtime.
...
docker compose up -d
pnpm install
cd implementation-by-spec/044-bedrock-runtime
pnpm setup
pnpm test
```
