# Codex CLI task - Cost Explorer

You are completing the `implementation-by-spec/049-cost-explorer/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to Cost Explorer.
...
docker compose up -d
pnpm install
cd implementation-by-spec/049-cost-explorer
pnpm setup
pnpm test
```
