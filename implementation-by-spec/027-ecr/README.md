...
> Container registry with repository lifecycle, scan-on-push, immutable tags, auth token, image listing, and cleanup.
...
docker compose up -d
pnpm install
cd implementation-by-spec/027-ecr
pnpm setup
pnpm seed
pnpm test
pnpm cleanup
...
- `src/client.ts` - ECR SDK v3 client for Floci (`http://localhost:4566`).
- `src/use-cases/repositories.ts - create repository, lifecycle policy, auth token, list images, delete.`
...
const repo = await createRepository("orders-api");
```
...
- Enable scan on push, immutable tags, lifecycle cleanup, cross-account policy, signing.
