...
> Serverless SQL over S3 with workgroups, query execution/status/results, external table SQL, and cleanup.
...
docker compose up -d
pnpm install
cd implementation-by-spec/033-athena
pnpm setup
pnpm seed
pnpm test
pnpm cleanup
...
- `src/client.ts` - Athena SDK v3 client for Floci (`http://localhost:4566`).
- `src/use-cases/queries.ts - create workgroups, start queries, get execution/results, delete workgroups, build external table SQL.`
...
- Partition data, compress/Parquet files, control result buckets, query costs, Glue permissions.
