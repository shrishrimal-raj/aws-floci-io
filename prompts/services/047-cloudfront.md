# Codex CLI task - CloudFront

You are completing the `implementation-by-spec/047-cloudfront/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to CloudFront.
...
docker compose up -d
pnpm install
cd implementation-by-spec/047-cloudfront
pnpm setup
pnpm test
```
