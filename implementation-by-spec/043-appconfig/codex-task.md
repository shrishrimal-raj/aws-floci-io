# Codex CLI task - AppConfig

You are completing the `implementation-by-spec/043-appconfig/` folder for an AWS + Floci learning monorepo.
...
2. **`src/use-cases/`** - at least two production patterns specific to AppConfig.
...
docker compose up -d
pnpm install
cd implementation-by-spec/043-appconfig
pnpm setup
pnpm test
```
